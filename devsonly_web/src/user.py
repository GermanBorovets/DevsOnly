...
def is_userset_owner(id: int, user: User) -> bool:
    # Function returns True if user is owner of user settings
    if userset_exists(id):
        return UserSettings.objects.get(id=id).user == user
    else:
        logger.error("This user settings don't exist")
        return False
